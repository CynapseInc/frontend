...
export default function CategoryCombobox({ value, defaultName = '', onChange, isErr }: CategoryComboboxProps) {
  ...
  const handleSelect = (category: CategoriaMov) => {
    onChange(String(category.id), category.descricao);
    setSelectedName(category.descricao);
    setOpen(false);
    setSearch('');
  };
              ...
              options.map((option) => (
                ...
                >
                  <div className="flex-1">
                    <div className="font-medium">{option.descricao}</div>
